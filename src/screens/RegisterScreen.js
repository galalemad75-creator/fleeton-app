// FleetOn — Register Screen (theme-aware)
...
import { signUp, createCompany } from '../lib/database';
...
export default function RegisterScreen({ navigation }) {
  ...
  const handleGroupCodeSubmit = async () => {
    if (groupCode.length !== 8) { Alert.alert(i18n.t('error'), 'Code must be 8 digits'); return; }
    setLoading(true);
    try {
      const result = await validateGroupCode(groupCode);
      if (!result.valid) {
        const reasonMap = { invalid_code: i18n.t('invalidCode'), limit_reached: i18n.t('codeExpired'), owner_exists: i18n.t('registrationClosed') };
        Alert.alert(i18n.t('error'), reasonMap[result.reason] || i18n.t('invalidCode'));
        return;
      }
      setSelectedRole({ ...selectedRole, key: result.role });
      setStep(3);
  ...
  const handleRegister = async () => {
    if (!name.trim() || !email.trim() || !password.trim()) { Alert.alert(i18n.t('error'), 'All fields are required'); return; }
    if (password !== confirmPassword) { Alert.alert(i18n.t('error'), 'Passwords do not match'); return; }
    if (password.length < 6) { Alert.alert(i18n.t('error'), 'Password must be at least 6 characters'); return; }
    setLoading(true);
    try {
      let companyId;
      if (selectedRole.isAdmin) {
        if (!companyName.trim()) { Alert.alert(i18n.t('error'), 'Company name is required'); setLoading(false); return; }
        const codes = await generateCompanyCodes();
        setGeneratedCodes(codes);
        const company = await createCompany({ name: companyName.trim(), driverCode: codes.driverCode, dispatcherCode: codes.dispatcherCode });
        companyId = company.id;
      } else {
        const result = await validateGroupCode(groupCode);
        companyId = result.company.id;
      }
      await signUp({ email: email.trim().toLowerCase(), password, name: name.trim(), role: selectedRole.key, companyId });
      if (selectedRole.isAdmin && generatedCodes) {
        Alert.alert(i18n.t('success'), `${i18n.t('shareCode')}\n\n${i18n.t('driverCode')}: ${generatedCodes.driverCode}\n${i18n.t('dispatcherCode')}: ${generatedCodes.dispatcherCode}`, [{ text: 'OK' }]);
      }
    } catch (err) { Alert.alert(i18n.t('error'), err.message); }
    finally { setLoading(false); }
  };
        ...
        <ScrollView contentContainerStyle={s.scrollContent} keyboardShouldPersistTaps="handled">
          ...
          {step === 1 && (
            <View style={s.stepContainer}>
              ...
              {ROLES.map((role) => (
                <TouchableOpacity key={role.key} style={[s.roleCard, selectedRole?.key === role.key && s.roleCardSelected]}
                  onPress={() => handleRoleSelect(role)} activeOpacity={0.7}>
                  <Text style={s.roleEmoji}>{role.emoji}</Text>
                  <View style={{ flex: 1 }}>
                    <Text style={s.roleLabel}>{i18n.t(role.label)}</Text>
                    {role.isAdmin && <Text style={s.roleSubtext}>{i18n.locale === 'ar' ? 'ينشئ حساب الشركة' : 'Creates the company account'}</Text>}
                  </View>
                </TouchableOpacity>
              ))}
            </View>
          ...
          {step === 2 && !selectedRole?.isAdmin && (
            <View style={s.stepContainer}>
              ...
              <Text style={s.stepSubtext}>
                {selectedRole?.key === 'driver' || selectedRole?.key === 'accountant' ? i18n.t('driverCode') : i18n.t('dispatcherCode')}
              </Text>
              <TextInput style={[s.input, s.codeInput]} placeholder="00000000" placeholderTextColor={COLORS.textMuted}
                value={groupCode} onChangeText={(t) => setGroupCode(t.replace(/[^0-9]/g, '').slice(0, 8))}
                keyboardType="number-pad" maxLength={8} textAlign="center" fontSize={28} letterSpacing={8} />
              <TouchableOpacity style={[s.button, (groupCode.length < 8 || loading) && s.buttonDisabled]}
                onPress={handleGroupCodeSubmit} disabled={groupCode.length < 8 || loading}>
                <Text style={s.buttonText}>{loading ? i18n.t('loading') : i18n.t('confirm')}</Text>
              </TouchableOpacity>
            </View>
          )}
          ...
          {step === 3 && (
            <View style={s.stepContainer}>
              {selectedRole?.isAdmin && (<>
                <Text style={s.label}>{i18n.t('name')} (Company)</Text>
                <TextInput style={s.input} placeholder="My Fleet Company" placeholderTextColor={COLORS.textMuted} value={companyName} onChangeText={setCompanyName} />
              </>)}
              <Text style={s.label}>{i18n.t('name')}</Text>
              <TextInput style={s.input} placeholder="Ahmed Mohamed" placeholderTextColor={COLORS.textMuted} value={name} onChangeText={setName} autoCapitalize="words" />
              <Text style={s.label}>{i18n.t('email')}</Text>
              <TextInput style={s.input} placeholder="you@example.com" placeholderTextColor={COLORS.textMuted} value={email} onChangeText={setEmail} keyboardType="email-address" autoCapitalize="none" />
              ...
              <TextInput style={s.input} placeholder="••••••••" placeholderTextColor={COLORS.textMuted} value={confirmPassword} onChangeText={setConfirmPassword} secureTextEntry />
              <TouchableOpacity style={[s.button, loading && s.buttonDisabled]} onPress={handleRegister} disabled={loading}>
                <Text style={s.buttonText}>{loading ? i18n.t('loading') : i18n.t('createAccount')}</Text>
              </TouchableOpacity>
            </View>
          )}
...
const styles = (COLORS) => StyleSheet.create({
  ...
  roleCard: { flexDirection: 'row', alignItems: 'center', backgroundColor: COLORS.surface, borderRadius: RADIUS.lg, padding: SPACING.lg, marginBottom: SPACING.md, borderWidth: 1, borderColor: COLORS.border },
